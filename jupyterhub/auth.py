...
from myproxy_login import myproxy_logon_py
...
class GSIAuthenticator(LocalAuthenticator):
    """Authenticate local Linux/UNIX users with PAM"""
    encoding = Unicode('utf8',
        help="""The encoding to use for PAM"""
    ).tag(config=True)
    ...
    open_sessions = Bool(True,
        ...
        It can be disabled with::
            ...
            c.GSIAuthenticator.open_sessions = False
        """
    ).tag(config=True)
    ...
    def authenticate(self, handler, data):
        ...
        try:
            ...
            if handler is not None:
                self.log.warning("GSI Authentication failed (%s@%s):", username, handler.request.remote_ip)
            else:
